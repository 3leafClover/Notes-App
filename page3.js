...
for (let i = 1; i <= 8; i++) {
    ...
    title.addEventListener("input", () => {
        localStorage.setItem(`storedTitle3${i}`, title.value);
        displaySlotTitles();
    });
}
...
function loadTitles() {
    for (let i = 1; i <= 8; i++) {
        titleElements[i - 1].value = localStorage.getItem(`storedTitle3${i}`);
    }
}
...
for (let i = 1; i <= 8; i++) {
    ...
    desc.addEventListener("input", () => {
        localStorage.setItem(`storedDesc3${i}`, desc.value);
    });
}

function loadDescs() {
    for (let i = 1; i <= 8; i++) {
        const descKey = `storedDesc3${i}`;
...
function saveColorsToLocalStorage(primaryColor, secondaryColor, borderColor) {
    localStorage.setItem("primaryColor3", primaryColor);
    localStorage.setItem("secondaryColor3", secondaryColor);
    localStorage.setItem("borderColor3", borderColor);
}

function loadColorsFromLocalStorage() {
    const storedPrimaryColor = localStorage.getItem("primaryColor3");
    const storedSecondaryColor = localStorage.getItem("secondaryColor3");
    const storedBorderColor = localStorage.getItem("borderColor3");
...
function saveBackgroundToLocalStorage(background) {
    localStorage.setItem("uploadedBackground3", background);
}

function loadBackgroundFromLocalStorage() {
    const background = localStorage.getItem("uploadedBackground3");
    if (background) {
