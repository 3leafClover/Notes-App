...
for (let i = 1; i <= 8; i++) {
    ...
    title.addEventListener("input", () => {
        localStorage.setItem(`storedTitle5${i}`, title.value);
        displaySlotTitles();
    });
}
...
function loadTitles() {
    for (let i = 1; i <= 8; i++) {
        titleElements[i - 1].value = localStorage.getItem(`storedTitle5${i}`);
    }
}
...
for (let i = 1; i <= 8; i++) {
    ...
    desc.addEventListener("input", () => {
        localStorage.setItem(`storedDesc5${i}`, desc.value);
    });
}

function loadDescs() {
    for (let i = 1; i <= 8; i++) {
        const descKey = `storedDesc5${i}`;
...
function saveColorsToLocalStorage(primaryColor, secondaryColor, borderColor) {
    localStorage.setItem("primaryColor5", primaryColor);
    localStorage.setItem("secondaryColor5", secondaryColor);
    localStorage.setItem("borderColor5", borderColor);
}

function loadColorsFromLocalStorage() {
    const storedPrimaryColor = localStorage.getItem("primaryColor5");
    const storedSecondaryColor = localStorage.getItem("secondaryColor5");
    const storedBorderColor = localStorage.getItem("borderColor5");
...
function saveBackgroundToLocalStorage(background) {
    localStorage.setItem("uploadedBackground5", background);
}

function loadBackgroundFromLocalStorage() {
    const background = localStorage.getItem("uploadedBackground5");
    if (background) {
