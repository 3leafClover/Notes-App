...
for (let i = 1; i <= 8; i++) {
    ...
    title.addEventListener("input", () => {
        localStorage.setItem(`storedTitle4${i}`, title.value);
        displaySlotTitles();
    });
}
...
function loadTitles() {
    for (let i = 1; i <= 8; i++) {
        titleElements[i - 1].value = localStorage.getItem(`storedTitle4${i}`);
    }
}
...
for (let i = 1; i <= 8; i++) {
    ...
    desc.addEventListener("input", () => {
        localStorage.setItem(`storedDesc4${i}`, desc.value);
    });
}

function loadDescs() {
    for (let i = 1; i <= 8; i++) {
        const descKey = `storedDesc4${i}`;
...
function saveColorsToLocalStorage(primaryColor, secondaryColor, borderColor) {
    localStorage.setItem("primaryColor4", primaryColor);
    localStorage.setItem("secondaryColor4", secondaryColor);
    localStorage.setItem("borderColor4", borderColor);
}

function loadColorsFromLocalStorage() {
    const storedPrimaryColor = localStorage.getItem("primaryColor4");
    const storedSecondaryColor = localStorage.getItem("secondaryColor4");
    const storedBorderColor = localStorage.getItem("borderColor4");
...
function saveBackgroundToLocalStorage(background) {
    localStorage.setItem("uploadedBackground4", background);
}

function loadBackgroundFromLocalStorage() {
    const background = localStorage.getItem("uploadedBackground4");
    if (background) {
