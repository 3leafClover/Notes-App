...
for (let i = 1; i <= 8; i++) {
    ...
    title.addEventListener("input", () => {
        localStorage.setItem(`storedTitle2${i}`, title.value);
        displaySlotTitles();
    });
}
...
function loadTitles() {
    for (let i = 1; i <= 8; i++) {
        titleElements[i - 1].value = localStorage.getItem(`storedTitle2${i}`);
    }
}
...
for (let i = 1; i <= 8; i++) {
    ...
    desc.addEventListener("input", () => {
        localStorage.setItem(`storedDesc2${i}`, desc.value);
    });
}

function loadDescs() {
    for (let i = 1; i <= 8; i++) {
        const descKey = `storedDesc2${i}`;
...
function saveColorsToLocalStorage(primaryColor, secondaryColor, borderColor) {
    localStorage.setItem("primaryColor2", primaryColor);
    localStorage.setItem("secondaryColor2", secondaryColor);
    localStorage.setItem("borderColor2", borderColor);
}

function loadColorsFromLocalStorage() {
    const storedPrimaryColor = localStorage.getItem("primaryColor2");
    const storedSecondaryColor = localStorage.getItem("secondaryColor2");
    const storedBorderColor = localStorage.getItem("borderColor2");
...
function saveBackgroundToLocalStorage(background) {
    localStorage.setItem("uploadedBackground2", background);
}

function loadBackgroundFromLocalStorage() {
    const background = localStorage.getItem("uploadedBackground2");
    if (background) {
